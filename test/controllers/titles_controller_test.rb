...
class TitlesControllerTest < ActionController::TestCase
  ...
  test "should create title" do
    assert_difference('Title.count') do
      post :create, title: { artist: @title.artist, format: @title.format, length: @title.length, size: @title.size }
    end
  ...
  test "should update title" do
    patch :update, id: @title, title: { artist: @title.artist, format: @title.format, length: @title.length, size: @title.size }
    assert_redirected_to title_path(assigns(:title))
  end
